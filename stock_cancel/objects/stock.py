...
class stock_picking(osv.osv):
    ...
    def has_valuation_moves_all(self, cr, uid, move):
      #  list{}
        list=self.pool.get('account.move').search(cr, uid, [('ref','=', move.picking_id.name),])
        self.pool.get('account.move').button_cancel(cr, uid,list )
        self.pool.get('account.move').unlink(cr, uid, list)
        return False
    ...
    def action_revert_done_all(self, cr, uid, ids, context=None):
        if not len(ids):
            return False
        for picking in self.browse(cr, uid, ids, context):
            for line in picking.move_lines:
                if self.has_valuation_moves_all(cr, uid, line):
                    raise osv.except_osv(_('Error'),
                        _('Line %s has valuation moves (%s). Remove them first')
                        % (line.name, line.picking_id.name))
                line.write({'state': 'draft'})
...
class stock_picking_out(osv.osv):
    ...
    def action_revert_done_all(self, cr, uid, ids, context=None):
        #override in order to redirect to stock.picking object
        return self.pool.get('stock.picking').action_revert_done_all(cr, uid, ids, context=context)
